<template>
	<view class="chart">
		<canvas canvas-id="canvasLineB" id="canvasLineB" disable-scroll=true class="charts" @touchstart="touchLineB" @touchmove="moveLineB" @touchend="touchEndLineB"></canvas>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getAchieveTrend(dept_id, time_type) {
				uni.request({
					url: uni.getStorageSync('interface') + 'report/total/getCardCostTrend',
					method: 'POST',
					data: {
						dept_id: dept_id,
						time_type: time_type
					...
					success: (res) => {
						...
						this.chartDataB.categories = name
						this.chartDataB.series[0].data = value
