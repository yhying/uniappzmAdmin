	...
	export default {
		...
		data() {
			return {
				...
				type: {
					index: 0,
					list: ['全部', '项目单', '产品单', '卡单', '充值单', '跨店', '散客']
				},
				status: {
					index: 0,
					list: ['全部', '未付款', '已付款', '分期', '分期中']
				},
				time: {
					index: 0,
		...
		methods: {
			...
			cartLists() {
				...
				if (this.time.start === '开始时间') {
					start = ''
				} else {
					start = this.time.start
				}
				if (this.time.end === '结束时间') {
					end = ''
				} else {
					end = this.time.end
				}
				uni.request({
					...
					method: 'GET',
					data: {
						pay_status:this.status.index,
						time_type: this.time.index,
						order_type: this.type.index,
						page: this.page.page,
