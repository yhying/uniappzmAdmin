<template>
	<view class="chart">
		<canvas canvas-id="canvasLineA" id="canvasLineA" disable-scroll=true class="charts" @touchstart="touchLineA" @touchmove="moveLineA" @touchend="touchEndLineA"></canvas>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getAchieveTrend(dept_id, time_type) {
				uni.request({
					url: uni.getStorageSync('interface') + 'report/total/getAchieveTrend',
					method: 'POST',
					data: {
						dept_id: dept_id,
						time_type: time_type
					...
					success: (res) => {
						...
						this.chartData.categories = name
